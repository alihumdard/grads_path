function switchTab(tabName) {
  ...
  if (tabName === "psych") {
    ...
    if (firstTab) {
      firstTab.classList.remove("bg-transparent", "text-[var(--text-muted)]");
...
function switchTherapyTab(tabName) {
  ...
  if (activeTab) {
    activeTab.classList.remove("bg-transparent", "text-[var(--text-muted)]");
...
function switchProgramsChart(tabName) {
  ...
  if (subtitleEl && subtitles[tabName]) subtitleEl.textContent = subtitles[tabName];
}

function switchProgTab(tabName) {
  const tabs = document.querySelectorAll(".prog-tab-btn");
  const inactiveClass = " bg-transparent text-[var(--text-muted)] hover:text-[var(--text-main)]";
